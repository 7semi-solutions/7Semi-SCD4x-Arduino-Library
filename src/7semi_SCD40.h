 ...
 * Version  : 1.0.0
 * Author   : 7Semi
 * Maintainer: info2@7semi.com
 * License  : MIT
 *
 * Features
 * --------
 * - Periodic and Low-Power periodic measurement
 * - Single-shot CO₂+RHT and RHT-only triggers
 * - Temperature offset, altitude, ambient pressure raw (compensation)
 * - ASC (Automatic Self-Calibration): enable/disable, target, periods
 * - Persist settings (NVM), read serial number & variant, self-test
 * - Power control: wake / power-down
 * - Flexible I²C: optional pin remap on ESP32/ESP8266; alternate TwoWire bus
 *
 * Notes
 * -----
 * - Keep I²C at 100 kHz for bring-up; higher is fine once stable.
 * - Each 16-bit payload word on the wire is followed by its own CRC byte.
 * - Temperature/Humidity conversion handled in readMeasurement():
 *     T(°C)  = -45 + 175 * raw / 65535
 *     RH(%)  = 100 * raw / 65535
 *     CO₂ppm = raw
 * - Set temperature offset only while the sensor is idle (not measuring).
 */
...
public:
   ...
   */
  bool begin(uint8_t i2cAddr = 0x62, int sda = -1, int scl = -1, uint32_t i2cFreq = 100000);
